...
번호는 **1단계에서 만드는 체크리스트 번호**를 쓴다. 사용자가 진행 상황으로 실제 보는 것이 그 목록이기 때문이다.
...
원인 가설은 세 SKILL.md 가 공통 출력 규약을 문제 상황에만 갖고 있다는 것입니다.
정상 흐름의 전이와 질문에는 규약이 없습니다.

사용자 대면 지점은 세 스킬 합쳐 17곳입니다. 목록은 계획 문서 3절에 있습니다.
...
현재 단계 — issue-start 11단계(이슈 리포트 코멘트) 완료
```
