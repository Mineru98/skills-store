...
var selected=null,mode='execution',query='',filters={status:{},labels:{},types:{}},doneCollapsed=false,activeEdgesOnly=false;
...
function reach(start,up){var set={};set[start]=1;var grew=true;while(grew){grew=false;edges.forEach(function(e){if(e.type!=='depends-on')return;var s=up?e.from:e.to,t=up?e.to:e.from;if(set[s]&&!set[t]){set[t]=1;grew=true}})}return set}
function chainSet(){if(selected==null||!by[selected])return null;var up=reach(selected,true),down=reach(selected,false),ids={},eks={};Object.keys(up).forEach(function(k){ids[k]=1});Object.keys(down).forEach(function(k){ids[k]=1});edges.forEach(function(e){if(e.type!=='depends-on')return;if((up[e.from]&&up[e.to])||(down[e.from]&&down[e.to]))eks[e.from+'>'+e.to]=1});return{nodes:ids,edges:eks}}
function drawEdges(){var svg=document.getElementById('edges'),lanes=document.getElementById('lanes');if(!svg||!lanes)return;var box=lanes.getBoundingClientRect(),chain=chainSet(),parts=['<defs><marker id="mk-dim" markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto"><path d="M0,0 L7,3.5 L0,7 z" fill="#9fb3bf"/></marker><marker id="mk-hot" markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto"><path d="M0,0 L7,3.5 L0,7 z" fill="#0d716b"/></marker></defs>'];svg.setAttribute('width',lanes.scrollWidth);svg.setAttribute('height',lanes.scrollHeight);edges.forEach(function(e){if(e.type!=='depends-on')return;var pre=by[e.to],dep=by[e.from];if(!pre||!dep)return;if(activeEdgesOnly&&(pre.status==='close'||dep.status==='close'))return;var a=lanes.querySelector('[data-node="'+Number(e.to)+'"]'),b=lanes.querySelector('[data-node="'+Number(e.from)+'"]');if(!a||!b)return;var ra=a.getBoundingClientRect(),rb=b.getBoundingClientRect(),sy=ra.top-box.top+ra.height/2,ty=rb.top-box.top+rb.height/2,sx,tx,mx,d;if(rb.left-ra.right>=8){sx=ra.right-box.left;tx=rb.left-box.left;mx=(sx+tx)/2;d='M'+sx+','+sy+' C'+mx+','+sy+' '+mx+','+ty+' '+tx+','+ty}else if(ra.left-rb.right>=8){sx=ra.left-box.left;tx=rb.right-box.left;mx=(sx+tx)/2;d='M'+sx+','+sy+' C'+mx+','+sy+' '+mx+','+ty+' '+tx+','+ty}else{sx=ra.right-box.left;tx=rb.right-box.left;d='M'+sx+','+sy+' C'+(sx+34)+','+sy+' '+(tx+34)+','+ty+' '+tx+','+ty}var hot=chain&&chain.edges[e.from+'>'+e.to];parts.push('<path d="'+d+'" class="edge '+(hot?'hot':chain?'cold':'')+'" marker-end="url(#mk-'+(hot?'hot':'dim')+')"/>')});svg.innerHTML=parts.join('')}
function card(n,rel){var state=execution.state(n),classes='node '+state+(selected===n.number?' selected':'')+(rel[n.number]?' related':''),number=Number(n.number),cause='';if(state==='blocked'){var unmet=((execution.pre||{})[n.number]||[]).filter(function(id){return !by[id]||by[id].status!=='close'});if(unmet.length)cause='<em class="cause">↳ #'+unmet.map(Number).join(', #')+' 완료 대기</em>'}return '<button class="'+classes+'" data-node="'+number+'"><b>#'+number+'</b><span>'+esc(n.title)+'</span><small>'+esc(state)+' · '+esc((n.labels||[]).join(', ')||'라벨 없음')+'</small>'+cause+'</button>'}
...
function executionView(list){if(!execution.can)return '<section class="blocked-execution"><h2>실행 순서를 계산할 수 없습니다</h2><p>'+esc(execution.cycle?'CYCLE: '+execution.cycle.join(' → '):((GRAPH.snapshot||{}).reason||'SNAPSHOT_REASON_UNAVAILABLE'))+'</p></section>';var chain=chainSet(),laneRel=chain?chain.nodes:{},lanes=[{key:'done',label:'완료',hint:'닫힌 이슈'},{key:'ready',label:'지금 착수',hint:'선행 없음 · 바로 시작'},{key:'in-progress',label:'진행 중',hint:'구현 진행'},{key:'blocked',label:'대기',hint:'선행 완료 필요'}];return '<div id="execution"><div class="lane-tools"><span class="lane-tools-title">레인 = 착수 가능 상태 · 선 = 의존 관계 (선행 → 후행) · 카드를 누르면 의존 체인이 강조됩니다</span><button id="toggle-done" class="chip'+(doneCollapsed?' on':'')+'">완료 레인 접기</button><button id="toggle-active-edges" class="chip'+(activeEdgesOnly?' on':'')+'">활성 엣지만</button></div><div id="lanes"'+(doneCollapsed?' class="done-collapsed"':'')+'>'+lanes.map(function(l){var items=list.filter(function(n){return execution.state(n)===l.key}),collapsed=doneCollapsed&&l.key==='done';return '<section class="lane '+l.key+(collapsed?' collapsed':'')+'"><div class="lane-head"><b>'+l.label+'</b><small>'+l.hint+'</small><strong>'+items.length+'</strong></div>'+(collapsed?'':'<div class="lane-body">'+items.map(function(n){return card(n,laneRel)}).join('')+'</div>')+'</section>'}).join('')+'<svg id="edges" aria-hidden="true"></svg></div><section class="paths"><h2>최장 실행 경로</h2>'+(execution.critical.length?execution.critical.map(function(path){return '<ol>'+path.map(function(id){return '<li>#'+esc(id)+' '+esc(by[id].title)+'</li>'}).join('')+'</ol>'}).join(''):'<p>미완료 이슈가 없습니다.</p>')+'</section></div>'}
...
export function outputPath(root, value) {
  ...
  const target = path.resolve(repository, value);
  ...
  const resolved = path.resolve(realpathSync(existing), path.relative(existing, target));
  const resolvedRelative = path.relative(repository, resolved);
  if (resolvedRelative === '..' || resolvedRelative.startsWith(`..${path.sep}`) || path.isAbsolute(resolvedRelative)) throw new Error('출력 경로는 저장소 안이어야 한다.');
  return target;
}
