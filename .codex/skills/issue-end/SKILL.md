...
이 스킬군은 한 작업을 여러 턴·여러 세션에 걸쳐 굴린다. 단계를 안 밝히면 사용자가 답하기 전에
"직전에 뭘 했더라" 부터 복원해야 한다. 되묻거나 스크롤을 올려 찾게 된다.
단계는 스킬이 이미 아는 정보다. 안 적을 이유가 없다.

대상은 둘이다. 문제 상황이 아니어도 해당한다.

```text
전이 보고   다음 단계로 넘어가기 직전, 무엇이 끝났고 무엇을 시작하는지 알릴 때
질문        승인·확인·선택을 받을 때 (AskUserQuestion 포함)
```

## 표기 형식

세 스킬(issue-start / issue-end / issue-merge)이 같은 형식을 쓴다.

```text
<스킬 이름> <n>단계(<단계 이름>)

예) issue-end 9단계(PR 생성)
```

번호와 이름은 아래 정본 표에서 그대로 가져온다. 즉석에서 지어내지 않는다.

## 단계 이름 정본

번호는 **1단계에서 만드는 체크리스트 번호**를 쓴다. 사용자가 진행 상황으로 실제 보는 것이 그 목록이고,
`references/report-and-pr.md` 가 이미 같은 번호로 5~9단계를 부른다.
아래 `# 실행 순서` 절의 `0단계` 머리말과 한 칸 어긋나지만, 표기의 정본은 이 표다.
...
줄 수는 사용자에게 보이는 본문 기준이다. AskUserQuestion 의 선택지는 세지 않는다.
경계에 걸리면 뒤에 붙인다. 긴 글은 끝에 있는 편이 진단에 쓸모 있다.
...
```text
증거를 기본 브랜치에 올렸습니다. mirrorRef 는 `main`, pushed 는 true 입니다.

이슈 #59 에 리포트를 코멘트했습니다. issue-start 가 남긴 코멘트가 이미 있어
새로 달지 않고 갱신했습니다.

이미지 4장이 모두 렌더링되는지 링크에서 확인해 주세요.

현재 단계 — issue-end 8단계(코멘트 렌더링 확인)
```

## 질문일 때

AskUserQuestion 을 쓸 때는 **질문 본문**에 단계를 넣는다. 선택지 라벨에는 넣지 않는다.
질문 본문은 짧으므로 거의 항상 앞에 붙는 형태가 된다.

```text
질문   issue-end 9단계(PR 생성)입니다. PR 을 만들까요?
```

필수 단계인 6·7단계는 묻지 않는다. 대신 **전이 보고로 단계를 밝히고 그대로 진행한다.**
선택지가 없는 것과 상태를 안 알리는 것은 다르다.

## 마무리 보고

마무리 보고 템플릿의 마지막 줄로 남긴다. 정상 종료했다는 것도 상태다.
