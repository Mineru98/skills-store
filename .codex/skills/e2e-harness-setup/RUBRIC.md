...
- **why**: 에이전트가 루프를 닫으려면 "성공이 무엇인지"가 검증 가능해야 한다(프로젝트 AGENTS.md 의 goal-driven 원칙). 약한 기준은 매번 사람에게 되묻게 만든다.
- **max_points**: 12
...
- **criterion**: 큰 모노레포 현실(앱별 인증/baseURL 차이, 기존 AGENTS.md/.codex/rules 존중, 계층형 AGENTS.md, 환경변수 baseURL, 토큰 비커밋)을 구체적으로 다룬다. 인접 E2E 스킬(generator/hardener/healer/ci-debug)로의 핸드오프 경계가 분명하다.
