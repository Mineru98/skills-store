...
description: issue-create, issue-start, issue-end, issue-merge에서 이슈 진행 상태가 성공적으로 전환된 직후 issue-onboard 그래프 캐시를 동기화합니다. `$issue-graph-sync #82 open`, "이슈 그래프 상태 동기화", "그래프에 진행 상태 반영" 요청과 issue-* 워크플로의 open·plan·in-process·review·close 전이 뒤에 사용합니다.
---

# Issue Graph Sync

`issue-onboard`에서 GitHub 이슈는 정본이고 `.issue/graph.json`은 재생성 가능한 캐시다.
...
2. 존재하면 현재 저장소의 `issue-onboard` 스킬을 찾아 아래를 실행한다.

   ```bash
   node <issue-onboard>/scripts/issue-onboard.mjs sync --state all
   ```

3. 결과가 완전한 스냅샷인지 확인한다. 성공하면 `GRAPH_SYNC=ok`, 이슈 번호, 상태,
   그래프 경로를 보고한다.
4. `issue-onboard`를 찾지 못했거나 sync가 실패·partial이면 경고와 사유만 남기고
