...
storageState 를 만들고 저장하는 **명령**은 여기서 다루지 않는다 — `.codex/skills/playwright-cli/references/storage-state.md`(`state-save` / `state-load`, 파일 포맷, 보안 주의). 이 문서는 그걸 **테스트 코드 구조에 어떻게 엮느냐**를 다룬다.
