...
merge 가 실패하면 출력의 `reason` 을 읽는다. 충돌이면 계획의 순서 판단이 틀린 것이다. 그 PR 을 보류로 돌리고 나머지를 계속할지 묻는다. 형식은 `references/ask.md` 3절을 따른다.

```text
질문 — issue-merge 6단계(merge · 통합 테스트)
PR #21 의 merge 가 실패했습니다(충돌). 나머지를 계속할까요?

1. 보류하고 계속 (권장)   이 PR 만 빼고 계획의 다음 순서로 넘어갑니다
2. 순서를 바꿔 재시도      먼저 넣을 PR 을 직접 알려주시면 다시 시도합니다
