...
- **why**: 스킬의 1차 발동 메커니즘은 description이다. Codex는 스킬을 과소발동하는 경향이 있어, 한국어 위주 레포에서 영어 표현까지 커버하지 못하면 실제로 안 불린다.
...
- **how to verify**: SKILL.md와 exploration-with-playwright.md가 `.codex/skills/playwright-cli/SKILL.md`를 참조하는지 확인. 노출된 playwright-cli 명령이 탐색용 최소 집합(open/snapshot/goto)으로 제한되고, storage-state 등 세부는 참조로 위임됐는지 확인.
