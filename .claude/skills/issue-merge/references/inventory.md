...
기본 브랜치의 주 체크아웃과 이 스킬이 만든 `.issue/merge/**` 보조 트리는 자동으로 빠진다.
...
브랜치 이름에 이슈 번호가 없거나 형식이 맞지 않는다. 숫자 앞은 `/` 나 `_` 여야 인식된다 — `fix/59-login` 은 되고 `fix-59-login` 은 안 된다.

이런 워크트리는 **통합 대상에서 제외한다.** 사용자에게 목록을 보여주고 이슈 번호를 알려줄지, 이번에는 건너뛸지 묻는다. 추측으로 이슈를 붙이지 않는다.
