...
이 스킬은 특히 더 그렇다. 이슈 여러 건을 한 번에 다루므로 어느 이슈의 어느 단계인지 함께 밝히지 않으면
사용자가 진행 상황을 머릿속으로 재구성해야 한다.
...
번호는 **0단계에서 만드는 체크리스트 번호**를 쓴다. 사용자가 진행 상황으로 실제 보는 것이 그 목록이다.
...
이 스킬처럼 회차 안에서 대상이 여러 개면 **대상 범위를 함께 적는다.**

```text
현재 단계 — issue-merge 6단계(merge · 통합 테스트) · #16 #21 #53 중 #53 처리 중
```
...
제외 3건은 기본 브랜치보다 앞선 커밋이 없어 합칠 것이 없습니다.
#64 는 before 증거가 없어 보류했습니다.

`#16` 과 `#21` 이 `src/shared/api.ts` 를 함께 건드립니다. merge 순서가 결과를 바꿀 수 있습니다.
...
```text
질문   issue-merge 6단계(merge · 통합 테스트)입니다. 아래 순서로 3건을 merge 할까요?
```
...
현재 단계 — issue-merge 7단계(이슈 close · 정리) 완료
```
