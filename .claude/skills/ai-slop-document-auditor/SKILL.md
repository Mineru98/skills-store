...
description: Check whether Korean document content feels AI-written across files such as PDF, Markdown, TXT, PPT/PPTX, HTML, DOCX, or pasted text by extracting text read-only, delegating to ai-slop-detector, ai-slop-guardrail, and optionally ai-slop-rewriter, then producing a concise audit verdict with evidence and remediation guidance.
---
...
Use this skill to inspect Korean document content and decide whether it has an AI-written feel. It is an orchestration skill: extract document text, preserve source locations, run detector and guardrail, and use the rewriter only when rewrites are requested or replacement copy is part of the deliverable.
...
Use the `Task` tool with `subagent_type: "ai-slop-detector"` for line-level tags.
...
REQUIRED SKILLS: ai-slop-detector.
...
Use `Task` with `subagent_type: "ai-slop-guardrail"` for a compact forbidden-expression and forbidden-structure list.
...
REQUIRED SKILLS: ai-slop-guardrail.
REQUIRED TOOLS: Read-only analysis only.
...
REQUIRED SKILLS: ai-slop-rewriter.
...
## Limits
- `<extraction gaps, unsupported pages, OCR uncertainty, or sampling limits>`
```
