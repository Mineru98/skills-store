...
- **max_points**: 12
- **how_to_verify**: SKILL.md 와 references 에서 `.claude/skills/playwright-cli/references/*.md` 경로 참조가 있는지. CLI 명령 나열(클릭/입력/state-save 문법 설명)을 자체적으로 다시 적지 않았는지 grep 으로 확인.
