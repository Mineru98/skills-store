# Skills Store — LangChain (Python)

Codex와 Claude Code 환경에서 사용하는 **LangChain · LangGraph · Deep Agents 스킬 세트**의 Python 버전입니다.

이 브랜치의 모든 코드 예시는 Python (`langchain`, `langgraph`, `deepagents`) 패키지를 기준으로 작성되어 있습니다. TypeScript 버전은 `langchain-typescript` 브랜치를 참고하세요.
...
| Skill | 설명 |
| --- | --- |
| `langchain-fundamentals` | `create_agent()`, `@tool`, 미들웨어 기반 프로덕션 에이전트 작성. |
| `langchain-middleware` | `HumanInTheLoopMiddleware`, 커스텀 미들웨어 훅, Pydantic 구조화 출력. |
...
| Skill | 설명 |
| --- | --- |
| `deep-agents-core` | `create_deep_agent()`, 하니스 아키텍처, SKILL.md 포맷, 설정 옵션. |
