...
변경 요청
  ...
  → issue-start   이슈 분석·구현·증거와 리포트 게시
  → 사람 검토·승인
  → issue-end     게시 상태 재확인·PR 생성
  → issue-merge   여러 PR 통합·재검증·이슈 종료
```

단일 작업은 `issue-create → issue-start → 검토·승인 → issue-end`로 진행하고, 준비된 PR의 merge·재검증·이슈 종료는 `issue-merge`로 마칩니다.
...
9. 게시된 증거와 리포트의 사람 검토·승인을 기다린 뒤 `issue-end`로 인계
...
`issue-end`는 `issue-start`가 증거와 리포트를 게시하고 사람이 승인한 작업을 PR로 마무리합니다.

기존 증거의 완결성과 원격 게시 상태를 다시 봅니다. 게시본이 로컬 증거와 같으면 반복 게시하지 않고 PR을 준비합니다. 부족하거나 바뀐 경우에만 before/after와 리포트를 보강해 재게시한 뒤 PR을 만듭니다.

`context`의 `evidencePublished`가 게시본과 로컬 증거의 일치 여부를 알려 줍니다. `false`면 PR 전에 기본 브랜치 또는 `evidence/issue-<번호>` 브랜치로 재게시하고 이슈 코멘트를 갱신합니다.
...
1. `context`로 워크트리·브랜치·이슈·PR·**증거 완결성과 게시 상태**를 판단
2. 증거가 부족하면 `pure-tree`로 변경 직전 상태를 만들어 before 재캡처
3. 게시본과 로컬 증거가 다를 때만 after·`comment.md` 보강
4. 필요 시 증거 커밋 → 브랜치 push → 기본/폴백 브랜치 재게시 → 이슈 코멘트 갱신
5. `issue-start`에서 받은 검토 승인을 확인
6. 자동 close 키워드 없이 관련 이슈 `#N`을 참조해 PR 생성
7. 다음 행동 4지선다

`issue-start`가 게시한 증거와 같으면 다시 미러하지 않습니다. 승인 뒤 증거가 바뀌었다면 게시본도 함께 갱신한 뒤 다시 승인을 확인합니다.
